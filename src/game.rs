...
use crate::engine;
use crate::engine::{Audio, Cell, Image, KeyState, Game, Point, Rect, Renderer, Sheet, Sound, SpriteSheet};
...
impl Game for WalkTheDog {
    ...
    fn draw(&self, renderer: &Renderer) {
        renderer.clear(&Rect::new_from_x_y(
            0,
            0,
            600,
            HEIGHT,
        ...
        renderer.draw_image(
            ...
            &Rect::new_from_x_y(
                (self.state_machine.context().position.x
                    + sprite.sprite_source_size.x).into(),
                (self.state_machine.context().position.y
                    + sprite.sprite_source_size.y).into(),
                sprite.frame.w.into(),
                sprite.frame.h.into(),
            ),
        );
